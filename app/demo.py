...
class Application:
    def __init__(self, window, window_title):
        ...
        self.prompt_string.set('Exlain the image in a single sentence.')
    ...
    def init_layout(self, window):
        self.img_canvas = Canvas(window, width = self.width, height = self.height)
        self.img_canvas.pack()

        self.prompt_textbox = Entry(window, textvariable=self.prompt_string)
        self.prompt_textbox.bind("<FocusOut>", lambda event: self.update_prompt())
        self.prompt_textbox.pack()

        self.response_label = Label(window)
        self.response_label.pack()

        self.live_checkbutton = Checkbutton(window, text='Live', style='Switch.TCheckbutton', command=self.toggle_live)
        self.live_checkbutton.pack()
        
        self.analysis_button = Button(window, text='Analyze', command = self.analyze)
...
app = Application(root, "Fixstars Demo")

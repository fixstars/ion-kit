...
Port::Impl::Impl()
    : id(PortID(sole::uuid4().str())), pred_chan{"", ""}, succ_chans{}, type(), dimensions(-1), is_dynamic_port(false)
{
}

Port::Impl::Impl(const NodeID & nid, const std::string& pn, const Halide::Type& t, int32_t d, const GraphID & gid)
    : id(PortID(sole::uuid4().str())), pred_chan{nid, pn}, succ_chans{}, type(t), dimensions(d), graph_id(gid), is_dynamic_port(false)
{
    params[0] = Halide::Parameter(type, dimensions != 0, dimensions, argument_name(id, nid, pn, 0, gid));
}

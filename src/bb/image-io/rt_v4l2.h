    ...
    for (int c = 0; c < 3; c++) {
        for (int dh = 0; dh < dst_height; dh++) {
            for (int dw = 0; dw < dst_width; dw++) {
                ...
                float x = ((static_cast<float>(dw) + 0.5f) * static_cast<float>(src_width)) / static_cast<float>(dst_width);
                x -= 0.5f;
                float y = (static_cast<float>(dh) + 0.5f) * static_cast<float>(src_height) / static_cast<float>(dst_height);
                y -= 0.5f;

...
public:
    ...
    void set_gain(int32_t sensor_idx, const std::string key, double v) {

        if (is_param_integer_) {
            set_gain(sensor_idx, key, static_cast<int32_t>(v));
            return;
        }
    ...
    void set_exposure(int32_t sensor_idx, const std::string key, double v) {

        if (is_param_integer_) {
            set_exposure(sensor_idx, key, static_cast<int32_t>(v));
            return;
        }
    ...
    void get_device_info(std::vector<void *> &outs) {
        if (sim_mode_ || operation_mode_ == OperationMode::Came2USB2 || operation_mode_ == OperationMode::Came1USB1) {
            ...
            ::memcpy(outs[0], &(devices_[device_idx_].header_info_), sizeof(ion::bb::image_io::rawHeader));
...
#undef GET_SYMBOL
    ...
    void open_real_devices(int32_t num_detected_device, int32_t num_usb_to_open, char *dev_id) {
        ...
        while (index_on_detected_device < num_detected_device && index_on_opened_device < num_usb_to_open) {
            ...
            if (strcmp(device_protocol, "USB3Vision") == 0) {
                ...
                if (operation_mode_ == OperationMode::Came1USB2 && index_on_detected_device > 0){
                    devices_[index_on_opened_device].u3v_payload_size_ = devices_[index_on_opened_device-1].u3v_payload_size_;
                    log::info("\tDevice/USB {}::{} : {} (Came1USB2;COPIED)", index_on_opened_device, "PayloadSize", devices_[index_on_opened_device].u3v_payload_size_);
                    devices_[index_on_opened_device].data_offset_ = devices_[index_on_opened_device-1].data_offset_;
                    devices_[index_on_opened_device].image_payload_size_ = devices_[index_on_opened_device-1].image_payload_size_;
                    if (is_gendc_){
                        devices_[index_on_opened_device].is_data_image_ = devices_[index_on_opened_device-1].is_data_image_;
                    }
                }else{
                    ...
                    if (is_gendc_) {
                        ...
                        char *buffer = reinterpret_cast<char *>(arv_device_dup_register_feature_value(devices_[index_on_opened_device].device_, "GenDCDescriptor", &gendc_desc_size, &err_));
                        if (err_) {
                            throw std::runtime_error(err_->message);
                        }
        ...
        while (true) {
            ...
            for (int i = 0; i < devices_.size(); ++i) {
                while (devices_[i].frame_count_ < max_cnt) {
                    ...
                    devices_[i].frame_count_ = frame_count_method_ == FrameCountMethod::TYPESPECIFIC3 ? static_cast<uint32_t>(get_frame_count_from_genDC_descriptor(bufs[i], devices_[i])) : frame_count_method_ == FrameCountMethod::TIMESTAMP ? static_cast<uint32_t>(arv_buffer_get_timestamp(bufs[i]) & 0x00000000FFFFFFFF) :
                                                                                                                                                                                                                                                  -1;
                    i == 0 ?
    ...
    uint32_t frame_cnt_;
    int32_t device_idx_;  // USB DEVICE INDEX
    ...
    std::vector<DeviceInfo> devices_;  // USB DEVICE
    ...
    void get(std::vector<Halide::Buffer<>> &outs) override {
        ...
        if (sim_mode_) {
            ...
            if (operation_mode_ == OperationMode::Came2USB2 || operation_mode_ == OperationMode::Came1USB1) {
                ...
                if (frame_sync_) {
                    sync_frame_count(bufs, timeout_us);
                }
                ...
                log::trace("Obtained Frame from USB{}: {}", device_idx_, frame_cnt_);
            }
        }
    }
...
private:
    U3VRealCam(int32_t num_sensor, bool frame_sync, bool realtime_display_mode, bool sim_mode, int32_t width, int32_t height, float_t fps, const std::string &pixel_format, char *dev_id = nullptr)
        ...
        auto num_device = arv_get_n_devices();
        if (num_device == 0) {
...
public:
    static U3V &get_instance(const std::string &id,
                             ...
                             const std::string &pixel_format = "Mono8") {
        if (instances_.count(id) == 0) {
            ion::log::info("Create  GenDC instance: {}", id);
            instances_[id] = std::unique_ptr<U3V>(new U3VGenDC(num_sensor, frame_sync, realtime_display_mode, sim_mode, width, height, fps, pixel_format));
        }
...
private:
    U3VGenDC(int32_t num_sensor, bool frame_sync, bool realtime_display_mode, bool sim_mode, int32_t width, int32_t height, float_t fps, const std::string &pixel_format, char *dev_id = nullptr)
        : U3V(num_sensor, frame_sync, realtime_display_mode, sim_mode, width, height, fps, pixel_format, nullptr) {
        // check if the camera is available
        arv_update_device_list();
        auto num_device = arv_get_n_devices();
...
extern "C" int ION_EXPORT u3v_dispose(const char *id) {
...
extern "C" int ION_EXPORT ion_bb_image_io_u3v_camera1(
    ...
    try {
        ...
        if (out0->is_bounds_query()) {
            ...
            // set gain & exposure
            u3v.set_gain(0, gain_key, gain0);
            u3v.set_exposure(0, exposure_key, exposure0);
...
extern "C" int ION_EXPORT ion_bb_image_io_u3v_camera2(
    ...
    try {
        ...
        if (out0->is_bounds_query() || out1->is_bounds_query()) {
            ...
            // set gain & exposure
            u3v.set_gain(0, gain_key, gain0);
            u3v.set_gain(1, gain_key, gain1);
            u3v.set_exposure(0, exposure_key, exposure0);
            u3v.set_exposure(1, exposure_key, exposure1);
...
extern "C" int ION_EXPORT ion_bb_image_io_u3v_camera2_frame_count(
    halide_buffer_t *,
    halide_buffer_t *,
    int32_t num_sensor, bool frame_sync, bool realtime_display_mode,
    halide_buffer_t *id_buf, halide_buffer_t *out) {
...
extern "C" int ION_EXPORT ion_bb_image_io_u3v_multiple_camera1(
    halide_buffer_t *id_buf,
    bool force_sim_mode,
    int32_t width, int32_t height, float_t fps,
    bool frame_sync, bool realtime_display_mode,
...
extern "C" int ION_EXPORT ion_bb_image_io_u3v_multiple_camera2(
    halide_buffer_t *id_buf,
    bool force_sim_mode,
    int32_t width, int32_t height, float_t fps,
    bool frame_sync, bool realtime_display_mode, bool enable_control,
...
extern "C" int ION_EXPORT ion_bb_image_io_u3v_multiple_camera_frame_count1(
    halide_buffer_t *,
...
extern "C" int ION_EXPORT ion_bb_image_io_u3v_multiple_camera_frame_count2(
    halide_buffer_t *,
    halide_buffer_t *,
    halide_buffer_t *id_buf, int32_t num_sensor,
    bool force_sim_mode,
...
extern "C" int ION_EXPORT ion_bb_image_io_u3v_device_info1(
    halide_buffer_t *,
    halide_buffer_t *id_buf, int32_t num_sensor,
    bool force_sim_mode,
    int32_t width, int32_t height, float_t fps,
...
extern "C" int ION_EXPORT ion_bb_image_io_u3v_device_info2(

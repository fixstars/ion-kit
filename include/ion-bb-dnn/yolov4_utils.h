...
std::vector<DetectionBox> post_processing(const float *boxes, const float *confs, const int num, const int num_classes, const float conf_thresh = 0.4, const float nms_thresh = 0.4) {
    ...
    std::vector<DetectionBox> yolo_boxes;
...
cv::Mat copy_with_boxes(const cv::Mat &frame, const std::vector<DetectionBox> &boxes, const int h, const int w) {
    cv::Mat new_frame;

...
extern "C" int ION_EXPORT ion_bb_image_io_grayscale_data_loader(halide_buffer_t *url_buf, halide_buffer_t *out) {

    try {
        const char *url = reinterpret_cast<const char *>(url_buf->host);
        if (end_with(url, ".zip")) {
            // We assume zip archived data ia sequence images
            static std::unordered_map<const char *, std::unique_ptr<miniz_cpp::zip_file>> zfiles;

        } else if (end_with(url, ".pgm")) {
            static std::unordered_map<const char *, cv::Mat> decoded;
            if (decoded.count(url) == 0) {
                std::string host_name;
                std::string path_name;

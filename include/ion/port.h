...
public:
    Port()
        : impl_(new Impl(NodeID(""), "", Halide::Type(), 0, GraphID(""))), index_(-1) {
    }

    Port(const std::shared_ptr<Impl> &impl, int32_t index)
     ...
     */
    Port(const std::string &n, Halide::Type t)
        : impl_(new Impl(NodeID(""), n, t, 0, GraphID(""))), index_(-1) {
    }
     ...
     */
    Port(const std::string &n, Halide::Type t, int32_t d)
        : impl_(new Impl(NodeID(""), n, t, d, GraphID(""))), index_(-1) {
    }
     ...
     */
    template<typename T>
    Port(const Halide::Buffer<T> &buf)
        : impl_(new Impl(NodeID(""), buf.name(), buf.type(), buf.dimensions(), GraphID(""))), index_(-1) {
        this->bind(buf);
    }
     ...
     */
    template<typename T>
    Port(const Halide::Buffer<T> &buf, const GraphID &gid)
        : impl_(new Impl(NodeID(""), buf.name(), buf.type(), buf.dimensions(), gid)), index_(-1) {
        this->bind(buf);
    }
     ...
     */
    template<typename T>
    Port(const std::vector<Halide::Buffer<T>> &bufs)
        : impl_(new Impl(NodeID(""), unify_name(bufs), Halide::type_of<T>(), unify_dimension(bufs), GraphID(""))), index_(-1) {
        this->bind(bufs);
    }
    ...
    const Channel &pred_chan() const {
        return impl_->pred_chan;
    }
    const NodeID &pred_id() const {
        return std::get<0>(impl_->pred_chan);
    }
    const std::string &pred_name() const {
        return std::get<1>(impl_->pred_chan);
    }
    const std::set<Channel> &succ_chans() const {
        return impl_->succ_chans;
    }
    const Halide::Type &type() const {
        return impl_->type;
    }
    ...
    int32_t size() const {
        return static_cast<int32_t>(impl_->params.size());
    }
    int32_t index() const {
        return index_;
    }
    ...
    // Util
    bool has_pred() const {
        return !std::get<0>(impl_->pred_chan).value().empty();
    }
    bool has_pred_by_nid(const NodeID &nid) const {
        return !to_string(std::get<0>(impl_->pred_chan)).empty();
    }
    bool has_succ() const {
        return !impl_->succ_chans.empty();
    }
    bool has_succ(const Channel &c) const {
        return impl_->succ_chans.count(c);
    }
...
private:
     ...
     */
    Port(const NodeID &nid, const std::string &pn)
        : impl_(new Impl(nid, pn, Halide::Type(), 0, GraphID(""))), index_(-1) {
    }

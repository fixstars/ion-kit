...
int main(int argc, char *argv[]) {
    try {
        ...
        auto n_txt = b.add("llm_llava")(n_img["output"]);
#if 0
...
#else
        ion::Buffer<uint8_t> text_output{1024};
        n_txt["output"].bind(text_output);

        b.run();

        std::cout << reinterpret_cast<const char*>(text_output.data()) << std::endl;
        #endif
